...
# File destination prefix
_FILES_DIR = "data/files"


def _build_document_dest_path(project_id: int, file_name: str) -> str:
    """Build document destination path."""
    return f"{_FILES_DIR}/{project_id}_{file_name}"


async def _download_document_file(
    ...
    await message.bot.download_file(file_path_obj.file_path, dest_path)
...
async def _save_and_create_document(
    ...
    dest_path = _build_document_dest_path(project_id, file_name)
    async for session in get_session():
        user = await get_user_by_telegram_id(session, message.from_user.id)
        await _download_document_file(message, dest_path)
        doc_data = {
...
@document_upload_router.callback_query(
    lambda c: c.data.startswith("upload_doc_task_"),
)
